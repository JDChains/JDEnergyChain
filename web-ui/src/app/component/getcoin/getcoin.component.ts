...
export class GetcoinComponent implements OnInit {
  ...
  success:string;
  constructor(private route: Router, private loader: NgxSpinnerService, private obj: FormBuilder, private getcoinService: GetcoinService
  ) {
  ...
  residentRegister() {
    ...
    this.getcoinService.submitser(args).subscribe(
      res => {
        this.errorrspn = "";
        console.log("first..");
  ...
  statusHouseowner() {
    ...
    this.getcoinService.getViewBank().subscribe(
      res => {
        this.values = res;
        console.log(res);
      },
